...
trait BinomialHeap extends Heap {
  ...
  protected def link(t1: Node, t2: Node): Node = // t1.r==t2.r
    if (ord.lteq(t1.x,t2.x)) Node(t1.x, t1.r+1, t2::t1.c) else Node(t2.x, t2.r+1, t1::t2.c) // decreasing ranks in children of binomial trees;
  protected def ins(t: Node, ts: H): H = ts match {
    case Nil => List(t)
    case tp::ts => // t.r<=tp.r
      if (t.r<tp.r) t::tp::ts else ins(link(t, tp), ts) // increasing rank in binomial queues
  }
  ...
  override def meld(ts1: H, ts2: H): H = (ts1, ts2) match {
    case (Nil, ts) => ts
    case (ts, Nil) => ts
    case (t1::ts1, t2::ts2) => //increasing rank in binomial queues:
