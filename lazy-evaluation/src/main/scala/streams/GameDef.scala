...
trait GameDef {
  ...
  case class Pos(row: Int, col: Int) {
    //A method named copy is implicitly added to every case class unless the class already
    //has a member (directly defined or inherited) with that name, or the class has
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    //
    def isStanding: Boolean = b1==b2 //b1.row == b2.row && b1.col == b2.col
